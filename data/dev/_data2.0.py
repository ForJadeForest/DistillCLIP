...
class DistillationDataModule(pl.LightningDataModule):
    def __init__(self, kwargs: Dict, dataset_list: List[Dataset], num_workers=8,
                 dataset='',
                 batch_size=128,
                 ):
        super().__init__()
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            self.trainset = self.instancialize(train=True, dataset_list=self.dataset_list)
            self.valset = self.instancialize(train=False, dataset_list=self.dataset_list)

        # Assign test dataset for use in dataloader(s)
        if stage == 'test' or stage is None:
            self.testset = self.instancialize(train=False, dataset_list=self.dataset_list)
