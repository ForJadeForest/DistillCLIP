...
def process_dataset(root_path, dataset_name, save_path):
    ...
    with multiprocessing.Pool() as pool, tqdm(total=len(image_paths), desc=f"Processing {dataset_name}") as progress:
        for _ in pool.imap_unordered(process_image, [(dataset_name, path, save_path) for path in image_paths]):
            progress.update()
...
dataset_name = ['VOC2010', 'imagenet1k', 'mscoco2017']
save_path = '/data/share/pyz/data/combine_data'

preprocess_images(root_paths, dataset_name, save_path)
