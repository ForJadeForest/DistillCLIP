...
from data.component.imagenet import ImageNetDataset
...
parser.add_argument(
    "--shards", default="/data/share/pyz/data/imagenet1k/train/shards", help="directory where shards are written"
)
parser.add_argument(
    "--data",
    default="/data/share/pyz/data/imagenet1k/",
...
def write_dataset(imagenet, base="./shards", split="train"):
    ...
    ds = ImageNetDataset(imagenet, True, need_label=True)
    nimages = len(ds)
    ...
    with wds.ShardWriter(pattern, maxsize=int(args.maxsize), maxcount=int(args.maxcount)) as sink:
        for i in indexes:
            ...
            key = "%07d" % i
            sample = {"__key__": key, "jpg": img, "cls": label}
