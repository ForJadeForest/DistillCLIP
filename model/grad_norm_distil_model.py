...
class GradNormDistillModel(pl.LightningModule):
    def __init__(self, student_encoder: torch.nn.Module,
        ...
        self.weights = torch.nn.Parameter(torch.ones(len(self.loss_control.loss_name)), requires_grad=True)
        self.weight_sum = self.weights.sum().detach()  # sum of weights
        self.alpha = 0.5
    ...
    def training_step(self, inputs, batch_idx, optimizer_idx):
        ...
        loss_order = []
        for n in self.loss_control.loss_name:
            loss_order.append(cal_res[n])
        loss_order = torch.stack(loss_order)
        if optimizer_idx == 0:
            weighted_loss = self.weights @ loss_order
            self.log_info('train_loss', weighted_loss, cal_res, batch_size=len(inputs))
            return weighted_loss
        elif optimizer_idx == 1:
            gw = []
            for i in range(len(loss_order)):
                dl = torch.autograd.grad(self.weights[i] * loss_order[i], self.student.output_layer.parameters(),
            ...
            grad_norm_loss = torch.abs(gw - constant).sum()

            with torch.no_grad():
                self.weights.data = self.weights.data / torch.norm(self.weights, p=1) * self.weight_sum
            self.log('weight1', self.weights.data[0])
            self.log('weight2', self.weights.data[1])
    ...
    def configure_optimizers(self):
        ...
        optimizer1 = optim.AdamW(opt_para, lr=self.hparams.lr, weight_decay=self.hparams.weight_decay)
        # optimizer = optim.SGD(self.parameters(), lr=self.hparams.lr, weight_decay=self.hparams.weight_decay)
        scheduler1 = transformers.get_cosine_schedule_with_warmup(
            optimizer1,
            num_warmup_steps=self.hparams.warm_steps,
            num_training_steps=self.hparams.total_steps
        )
        optimizer2 = optim.AdamW([self.weights] + list(self.student.output_layer.parameters()), lr=0.1)
        return (
            {
