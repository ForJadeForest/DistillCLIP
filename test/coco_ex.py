...
def main():
    ...
    clip_model, _ = load('ViT-B/32', 'cuda', r'/data/pyz/.cache')
    clip_model.half()
    clip_model.cuda()
    clip_model.eval()
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    images_root_dir = Path(r'/data/pyz/data/mscoco/val2014')
    image_paths, images_features = init_image_features(clip_model, images_root_dir, device)
    ...
    m1_spearmanr, m1_p_value = scipy.stats.spearmanr(clip_score_res, human_metric_res_m1)
    print(f'CLIPScore for M1 Spearmanr: {m1_spearmanr}, p-value: {m1_p_value}')
    m2_spearmanr, m2_p_value = scipy.stats.spearmanr(clip_score_res, human_metric_res_m2)
